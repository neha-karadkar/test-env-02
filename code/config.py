...
class Config:
    ...
    def _load_keyvault_secrets(cls):
        ...
        try:
            ...
            import json
            secret_map = getattr(cls, "KEY_VAULT_SECRET_MAP", [])
            by_secret = defaultdict(list)
            for attr, ref in secret_map:
                if "." in ref:
...
def _initialize_config():
    ...
    CONFIG_VARIABLES = [
        ...
        "LLM_TEMPERATURE",
        "LLM_MAX_TOKENS",
        # LLM Model List (for observability)
        "LLM_MODELS",
        # API Keys
        "OPENAI_API_KEY",
    ...
    for var_name in CONFIG_VARIABLES:
        ...
        if var_name == "LLM_MODELS":
            if value:
                ...
                value = []
        # Special handling for CONTENT_SAFETY_ENABLED (bool)
        elif var_name == "CONTENT_SAFETY_ENABLED":
            if value is not None:
                value = str(value).lower() in ("true", "1", "yes")
            else:
                value = False
        # Special handling for CONTENT_SAFETY_SEVERITY_THRESHOLD (int)
            ...
            if value:
                ...
                value = 2
        # Special handling for LLM_TEMPERATURE (float)
            ...
            if value:
                ...
                value = None
        # Special handling for LLM_MAX_TOKENS and OBS_AZURE_SQL_PORT (int)
            ...
            if value:
                ...
                value = None
        # Special handling for OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE (default "yes")
        elif var_name == "OBS_AZURE_SQL_TRUST_SERVER_CERTIFICATE":
            if value is None or value == "":
                value = "yes"
        # Everything else: warn if missing
